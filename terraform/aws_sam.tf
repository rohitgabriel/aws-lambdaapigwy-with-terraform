...
resource "aws_s3_bucket_object" "sam_deploy_object" {
  ...
  key    = "sam-deploy-templates/${var.app_name}-${var.stage_name}-deploy-${timestamp()}.yaml"
...
resource "aws_cloudformation_stack" "employees_api_sam_stack" {
  name         = "${var.app_name}-${var.stage_name}-sam-stack"
  capabilities = ["CAPABILITY_NAMED_IAM", "CAPABILITY_AUTO_EXPAND"]
  parameters = {
    AppName = "${var.app_name}-${var.stage_name}"
    TableName = "${var.app_name}-${var.stage_name}"
  }
