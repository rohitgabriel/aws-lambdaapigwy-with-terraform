...
resource "aws_iam_role" "apigateway_execution_role" {
  name               = "${var.app_name}-${var.stage_name}-apigateway-role"
  path               = "/"
...
resource "aws_iam_policy" "lambda_permissions_policy" {
  name   = "${var.app_name}-${var.stage_name}-lambda-permissions"
  path   = "/"
...
resource "aws_api_gateway_rest_api" "employees_rest_api" {
  ...
  body = templatefile("./files/api-def-v1.yaml", {
    app_name                   = "${var.app_name}-${var.stage_name}",
...
# ----------------------------------------------------------------------
# API Gateway Another Stage 
# ----------------------------------------------------------------------
resource "aws_api_gateway_stage" "test" {
  stage_name    = var.stage_canary
  depends_on  = [aws_api_gateway_rest_api.employees_rest_api]
  rest_api_id   = aws_api_gateway_rest_api.employees_rest_api.id
  deployment_id = aws_api_gateway_deployment.employees_rest_api_deployment.id
}
...
resource "aws_api_gateway_api_key" "api_key" {
  name        = "${var.app_name}-${var.stage_name}-key"
...
resource "aws_api_gateway_usage_plan" "usage_plan" {
  name        = "${var.app_name}-${var.stage_name}-usage-plan-${timestamp()}"
  depends_on  = [aws_api_gateway_rest_api.employees_rest_api]
